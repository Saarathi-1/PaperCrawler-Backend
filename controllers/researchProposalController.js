...
const createResearchProposal = async (req, res) => {
    try {
        ...
        let researchProposal = await ResearchProposal.create({
            user_id,
            title,
            researchers,
            keywords,
...
const editResearchProposal = async (req, res) => {
    try {
        ...
        // await ResearchProposal.updateOne({_id:research_proposal_id,})
        if (funding_amount != "") {
            await ResearchProposal.updateOne({_id:research_proposal_id},[{$set:{funded:true}}])
        }
...
const getAllSubmittedResearchProposalsByUser = async (req, res) => {
    try {
        ...
        const funded = await ResearchProposal.find({ user_id, submitted: true, funded: true })
        const drafts = await ResearchProposal.find({ user_id })
        const verified = await ResearchProposal.find({ user_id, verified: true })

        const submitted_count = researchProposals.length;
        const funded_count = funded.length;
        const drafts_count = drafts.length;
        const verified_count = verified.length;

        return res.json({
            success: true,
            message: "Research Proposal Fetched Successfully",
